...
class StandaloneHiveServerContext implements HiveServerContext {
    ...
    protected void configureFileSystem(TemporaryFolder basedir, HiveConf conf) {
        ...
        createAndSetFolderPropertyWithUris("hadoop.tmp.dir", "hadooptmp", conf, basedir);
        createAndSetFolderProperty("test.log.dir", "logs", conf, basedir);
        createAndSetFolderPropertyWithUris("hive.vs", "vs", conf, basedir);
    }
    ...
    protected final void createAndSetFolderProperty(HiveConf.ConfVars var, String folder, HiveConf conf,
        ...
        File confFolder = newFolder(basedir, folder);
        conf.setVar(var, confFolder.getAbsolutePath());
    }

    protected final void createAndSetFolderProperty(String key, String folder, HiveConf conf, TemporaryFolder basedir) {
        File confFolder = newFolder(basedir, folder);
        conf.set(key, confFolder.getAbsolutePath());
    }

    /**
     * Use this method instead of `createAndSetFolderProperty` when you need to access uri paths from windows.
     * @param key
     * @param folder
     * @param conf
     * @param basedir
     */
    protected final void createAndSetFolderPropertyWithUris(String key, String folder, HiveConf conf, TemporaryFolder basedir) {
        File confFolder = newFolder(basedir, folder);
        String path = confFolder.toURI().getPath();
        String scheme =  confFolder.toURI().getScheme() + "://";

        conf.set(key, confFolder.getAbsolutePath());
        conf.set(key + ".uri.path", path);
        conf.set(key + ".uri.scheme", scheme);
        conf.set(key + ".uri", scheme + path);
    }
