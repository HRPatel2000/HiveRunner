...
public class HelloHiveRunner {
    ...
    public Map<String, String> hiveProperties = MapUtils.putAll(new HashMap(), new Object[]{
            "MY.HDFS.DIR", "${hadoop.tmp.dir.uri.path}",
            "my.schema", "bar",
    });
     ...
     */
    @HiveResource(targetFile = "${hiveconf:hadoop.tmp.dir}/foo/data_from_string.csv")
     ...
     */
    @HiveResource(targetFile = "${hiveconf:hadoop.tmp.dir}/foo/data_from_file.csv")
    ...
    public void testSelectFromCtas() {
        ...
        List<String> actual = hiveShell.executeQuery("select a.s from (select s, i from foo_prim order by i) a");//TODO: Fix OrderBy.  order by i
        Assert.assertEquals(expected, actual);
    }
