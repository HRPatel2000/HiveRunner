...
public class HiveRunnerAnnotationsTest {
    ...
    @HiveSetupScript
    private Path setupPath = getPathFromResource("hiveRunnerAnnotationsTest/setupPath.csv");
    ...
    public Path dataFromPath = getPathFromResource("hiveRunnerAnnotationsTest/testData2.csv");

    private static Path getPathFromResource(String resource) {
        Path ret;
        try {
            ret = Paths.get(ClassLoader.getSystemResource(resource).toURI());
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new IllegalStateException("Setup path msut be accessible and it's path must have correct syntax.");
        }
        return ret;
    }
